...
# Preprocess the data
df['text'] = df['overview'].astype(str) + ' ' + df['genres'].astype(str)

# Encode the movie titles as numerical labels
label_encoder = LabelEncoder()
df['label'] = label_encoder.fit_transform(df['keywords'])
...
predicted_keyword = label_encoder.inverse_transform([predicted_label.argmax()])
print('Predicted Movie:', predicted_keyword)
print('Predicted Movie Overview:', df[df['keywords'] == predicted_keyword[0]]['title'].values[0])